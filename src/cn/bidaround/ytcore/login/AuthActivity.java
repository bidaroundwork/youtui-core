...
public final class AuthActivity extends YtBaseActivity {
	...
	private void initData() {
		...
		if ("sina".equals(flag)) {
			mWeiboAuth = new WeiboAuth(this, KeyInfo.sinaWeibo_AppKey, KeyInfo.sinaWeibo_RedirectUrl, YoutuiConstants.SINA_WEIBO_SCOPE);
	 ...
	 */
	private void initQQ() {
		mTencent = Tencent.createInstance(KeyInfo.qQ_AppId, this);
	...
	private void initTencentWb() {
		...
		if (!Util.isNetworkAvailable(this)) {
			...
			try {
				clientId = KeyInfo.tencentWeibo_AppKey;
				redirectUri = KeyInfo.tencentWeibo_RedirectUrl;
	...
	Handler handle = new Handler() {
		...
		public void handleMessage(Message msg) {
			...
			case GET_USERINFO_TENCENTWB:
				...
				new Thread() {
					public void run() {
						String url = "http://open.t.qq.com/api/user/info?format=json" + "&openid=" + tencentWbOpenid + "&oauth_consumer_key=" + KeyInfo.tencentWeibo_AppKey + "&access_token=" + tencentWbAccessToken + "&clientip=" + Util.getLocalIPAddress(AuthActivity.this) + "&oauth_version=2.a&scope=" + YoutuiConstants.TENCENT_SCOPE;
